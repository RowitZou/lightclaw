...
LightClaw 是一个用 TypeScript 从头编写的自托管 AI Agent Harness，以 [Claude Code](https://github.com/anthropics/claude-code) 为架构蓝本。核心是一个轻量、终端原生的 REPL，背后连接流式工具调用的 agent loop、持久化 session、自动上下文压缩、Memory + Skill 系统、子 Agent、Web 工具，以及 MCP 工具。

## 当前状态

Phase 1 – 6 全部完成。构建产物为 ~126 KB 的单文件 ESM bundle。已具备：
...
刻意**未**实现：React/Ink 全屏 UI、hooks、MCP Server 模式、MCP resources/prompts/OAuth/sampling、消息渠道（飞书/微信/IDE Bridge）、`@include` 指令、fork-agent memory 提取、micro-compact、`allowed_tools` 强制执行、进程沙箱。
