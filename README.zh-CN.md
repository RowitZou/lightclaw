...
Phase 1 – 8 Step 13 已完成。构建产物为 ~200 KB 的单文件 ESM bundle。已具备：
...
- **飞书渠道**：webhook-only daemon（`lightclaw channel feishu start`），支持文本收发、媒体接收、去重、allowlist、session 路由、代理感知 SDK client 与非交互权限
...
刻意**未**实现：Phase 8 真实飞书 / 微信 / 双渠道联调、React/Ink 全屏 UI、MCP Server 模式、MCP resources/prompts/OAuth/sampling、IDE Bridge 渠道、飞书 WebSocket/streaming card/出站媒体、微信出站媒体/多账号/typing indicator、`@include` 指令、fork-agent memory 提取、micro-compact、`allowed_tools` 强制执行、进程沙箱。
