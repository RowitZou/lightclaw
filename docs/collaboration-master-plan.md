...
>
> **根级镜像(2026-06-10 补,idle-root)**:非终态的**根**,名下义务全部结清、无排期、空闲超宽限 → 「目标开着但没有任何东西朝它移动」→ 唤醒 main(派下一步或关账)。这堵住 orchestrator 自身的推进死角——worker 停摆账面可见,而「下一步只活在 main 计划里」原本零 finding;该谓词仍是纯死活(账面可判)。常驻服务根排除(构造上总有 queued 子)。
...
**催办唤醒的目标分期(2026-06-10)**:验收**权**永远沿树边逐级(父收子,§6.1 不变),但「催办唤醒」的落点分期——phase1 只有 main 一个可唤入口(父 worker 班次结束即蒸发,resume 未落地),所以现状是父没收 → 唤 main 走 orchestrator fallback 代收;**phase3 resume 落地后催办改 parent-first**:先唤直接父级续班次就地验收,父级也推不动再按 §8.3 逐层升到 main。看门狗的 suppress 判据已按此对齐(delivered 的「接收方在忙」看的是**父**的 session——父还活着就不催,等它自己收)。
...
精确尾巴:**「找得到」≠「唤醒便宜」。** 唤醒一个冷的父就为转交一条消息 = 一整轮 LLM。所以不靠「保持暖」省钱,而靠**路由**(约束 C,2026-06-10 修订:没醒就唤醒、醒了就插嘴——唤醒已在途时后续消息自然并入那个 turn,不为每条消息单独付一轮;不引入 lazy 攒批)。

### 6.5 不变量 B + 超时逃生阀

> **不变量 B:子 agent 在「发出上行消息后、收到下行回复前」,不能把工单设为完成。**

保证消息能送达。但**必须加超时**:父可能永远不回(父自己崩了 / 问题已作废)。**ask 是 turn 内等待的工具调用**(2026-06-10 实现回归本意:发问的 run 保持 running,答案或默认作为工具返回,班次不中断;迟到的答复走普通消息)。**超时默认 15 分钟**(2026-06-10 自 ~10 分钟上调:未决项可能逐层上报到用户,用户卡超时 10 分钟,留 5 分钟传导余量,避免用户最后一刻拍板时 worker 已按默认开跑)。发问的 agent 必须自带默认选项,超时即按默认续做。否则子被永久钉死。

**ask 通道全员化(2026-06-11,026ec4a)**:每个非 internal 角色都持有 Message——被派出去的角色必有 requester 可问,叶子(localExplorer/webSearcher)亦不例外。叶子的 Message **仅上行 by construction**(无 Dispatch 即无子,下行有所有权护栏拒掉一切非直接子),授予恰好等于 ask 通道。同 commit 补齐 archivist 的 dispatcher kit(TaskUpdate/Sleep——4e1ad69 修 coder/reviewer/feishuSecretary 时的同类遗漏:能派子却不能验收),两条 roster 不变量(非 internal 必有 Message;有 Dispatch 必有全套件)进 `role-tool-gate.test.ts`,下次加角色漏件直接红。

### 6.6 用户叫停(/stop):停手≠销单(2026-06-10)
...
| 上行问 + 下行控 | 两端管子全现成:给活 worker 递消息 = interjection 队列 + tool-boundary drain;叫醒不在线的 main = background-result 合成 turn;Cancel/Update 工具壳在 | Cancel 补**真 abort**(现在只删记录);接「worker 问 → 叫醒 main → 回答 → 回灌」这条线(新 subscriber,两头现成);约束 C 的**唤醒路由**(没醒就唤醒、醒了就插嘴) |
