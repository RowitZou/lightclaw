...
function modelCustomUsage(): string {
  return [
    ...
    '  /model custom add <model> <anthropic|openai|openai-auth> <endpoint> <upstreamModel> [--reasoning <none|minimal|low|medium|high|xhigh>] [--max-output-tokens <n>] [--no-default] [--timeout-ms <n>]',
    '  /model custom set <model> [--schema <anthropic|openai|openai-auth>] [--endpoint <endpoint>] [--upstream-model <id>] [--reasoning <none|minimal|low|medium|high|xhigh|->] [--max-output-tokens <n|->] [--timeout-ms <n>]',
...
async function addModel(userId: string, parts: string[], ctx: ReplContext): Promise<string> {
  ...
  const reasoningEffort = parseReasoningEffort(flagValue(rest, '--reasoning'))
  const maxOutputTokens = parseOptionalPositiveInt(flagValue(rest, '--max-output-tokens'))
  ...
  return `${[
    ...
    ...models
      ...
      .map(([name, model]) => {
        ...
        return `  ${name} (${model.schema}, ${model.endpoint} -> ${model.upstreamModel})${current}`
      }),
    '',
  ].join('\n')}`
}
