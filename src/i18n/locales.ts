...
export const LOCALES = {
  cn: {
    ...
    'card.admin.user.badge.admin': '⭐ 管理员',
    'card.admin.user.badge.ceiling': '上限 {mode}',
    ...
    'user.unlink.invalidKey': '{link} 不是有效的渠道标识（格式应为 channel:id）。',
    ...
    'channel.failure.rateReason': '原因：上游限流或用量额度达到上限（已自动重试 2 次仍失败）',
    'channel.failure.rateRepeatBrief': '⏳ 模型 {model} 仍处于限流 / 额度上限，本条未处理（额度恢复后重发即可）。',
    ...
    'mount.pathNotAbsolute': '挂载路径必须是绝对路径：{path}',
    'mount.notUnderSharedRoot':
      '该路径不在任何可挂载的共享存储根目录下：{path}。可挂载的根目录：{roots}。请改挂其中某个根目录之下的子目录。',
  ...
  en: {
    ...
    'card.admin.user.badge.admin': '⭐ admin',
    'card.admin.user.badge.ceiling': 'ceiling {mode}',
    ...
    'user.unlink.invalidKey': '{link} is not a valid channel link (expected format: channel:id).',
    ...
    'channel.failure.rateReason': 'Reason: upstream rate limit or usage cap reached (auto-retried twice, still failed)',
    'channel.failure.rateRepeatBrief': '⏳ Model {model} is still rate-limited / over its usage cap; this message was not processed (resend after the quota window resets).',
    ...
    'mount.pathNotAbsolute': 'Mount path must be absolute: {path}',
    'mount.notUnderSharedRoot':
      'This path is not under any mountable shared-storage root: {path}. Mountable roots: {roots}. Mount a sub-directory under one of these roots instead.',
