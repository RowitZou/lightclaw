...
function formatDispatchModeSectionWorker(): string {
  return [
    '## Dispatch Mode',
    'Specialist work belongs to specialists: when a piece clearly belongs to another specialty, or is heavy enough that doing it inline would crowd out your own thread, hand it to the right worker. Otherwise, keep a short, bounded check inline when your own tools can settle it reliably — for example, checking a fact for which you already have the sources or re-reading something you produced. Every dispatch runs in the background — the call returns immediately, and the result reaches you later as a <background-task-result>.',
    '- Independent pieces? Send them as separate dispatches in one message; results arrive as each finishes.',
...
const DISCIPLINE_BLOCKS: DisciplineBlock[] = [
  {
    ...
    bullets: [
      ...
      { text: '- Deliver the outcome in the reply you end your turn on — that closing reply is the answer the user reads. When you finish a goal, settle the ledger and close the root first, then give the user the result as that final reply; don\'t fold your conclusion into an earlier message that still goes on to make a tool call.' },
  ...
  },
  {
    ...
    bullets: [
      ...
      { text: '- Your final reply is what your requester receives as your result — the progress notes you leave between steps are not part of it. Put the complete outcome in the reply you end your turn on: do your closing bookkeeping first, then write the full result there, rather than spreading it across earlier notes.' },
  ...
  },
  {
    ...
    bullets: [
      ...
      { text: '- The reply you end your turn on is your deliverable — make it carry the complete result (what changed, what you found, where it lives), not a one- or two-sentence recap. Keep it as long as the task needs and no longer: a trivial reply still stays short.' },
  ...
  },
  {
    ...
    bullets: [
      ...
      { when: { allOf: [{ kind: 'worker' }, { dataPresent: 'reachableWorkers' }] }, text: '- When ## Reachable Workers is rendered above, the work is still yours by default — you own it end to end. Hand off a piece when it clearly belongs to another specialty or is too heavy to carry inline. Otherwise, settle short, bounded checks with your own tools when you can do so reliably. What comes back is an input you verify and integrate — handing off a piece never hands off the responsibility, and farming out the whole task is not delegation, it is abdication.' },
