  ...
  await withTempHome(async home => {
    ...
    await runWithSessionContext(ctx, async () => {
      ...
      const theirs = await readFile(
        path.join(home, 'users', 'alice', 'skills', 'their-flow', 'SKILL.md'), 'utf8')
      ...
      const mine = await readFile(
        path.join(home, 'users', 'alice', 'skills', 'default-flow', 'SKILL.md'), 'utf8')
...
test('revising an existing skill preserves its roles and refuses non-members', async () => {
  await withTempHome(async home => {
    ...
    const skillPath = path.join(home, 'users', 'alice', 'skills', 'shared-flow', 'SKILL.md')
