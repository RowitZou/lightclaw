...
describe('deferred loading policy', () => {
  it('keeps every tool inline when mode is off', () => {
    ...
    const result = buildTurnToolCatalog({
      ...
      config: fakeConfig({ webSearch: {}, webFetch: { preapprovedDomains: [] }, maxOutputBytes: 51_200, catalog: { deferredLoading: 'off', deferredLoadingThreshold: 1, discoveredToolsMaxSize: 30, discoveredToolsTtlTurns: 20 } }),
    })
  ...
  it('enables auto mode at the threshold and injects ToolSearch', () => {
    ...
    const result = buildTurnToolCatalog({
      allTools: tools,
      discoveredTools: new Map(),
      config: fakeConfig({ webSearch: {}, webFetch: { preapprovedDomains: [] }, maxOutputBytes: 51_200, catalog: { deferredLoading: 'auto', deferredLoadingThreshold: 2, discoveredToolsMaxSize: 30, discoveredToolsTtlTurns: 20 } }),
    })
  ...
  it('adds discovered deferred tools to the next-turn catalog', () => {
    ...
    const result = buildTurnToolCatalog({
      ...
      config: fakeConfig({ webSearch: {}, webFetch: { preapprovedDomains: [] }, maxOutputBytes: 51_200, catalog: { deferredLoading: 'always', deferredLoadingThreshold: 30, discoveredToolsMaxSize: 30, discoveredToolsTtlTurns: 20 } }),
    })
