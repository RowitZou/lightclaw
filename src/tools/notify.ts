...
## When to use Notify (✅) vs not (❌)
✅ User explicitly asked to be pinged when X happens, and X happened.
✅ A scheduled task you dispatched has hit a critical threshold or failed in a way that requires user action.
✅ Security / privacy / data-integrity alert.
✅ A user-set condition has been triggered ("notify me when NVDA crosses 200").
❌ "Web research is done, here's the result" — that's a plain reply.
❌ Routine progress updates ("step 3 of 5 complete") — that's progress signal territory, not Notify.
❌ FYI information the user might find interesting — plain reply or stay silent.
❌ Cron task fired successfully as expected — plain reply if anything, often stay silent.
❌ Self-acknowledgment / meta-commentary about what you just did — never goes through Notify.
