...
3. **Assemble the spec from your library; default or ask for what's missing.**
   ...
   - **Environment** — in order: reuse a working env already in `/workspace`; else use the env the image ships if it works; if neither works — or the one you'd use is missing a backend or library the run needs that you can't add yourself — → *(blocker)*. Don't jump straight to building a whole env when a usable one is already there.
...
- **Settle the environment before you submit — and never let the job install it.** Take what already works, in order: an env in `/workspace`, then the env the image ships. Building your own is not a default you reach for here — it's what the *No usable environment* blocker falls back to if the user doesn't answer. When you do build it, **follow the project's own install instructions** (its README / `requirements` / install script) as one coherent install — don't assemble it package-by-package reacting to one `ImportError` at a time, the half-broken-env trap. Build from a prep job in the target image, into `/workspace`, complete and self-contained, and prove it with a smoke check before the real run. Use the image's env directly — don't link or copy it into `/workspace`: a `/workspace` env that's secretly a `--system-site-packages` venv over the image's conda only resolves inside that image (it lists but cannot run outside it). Never bake `pip install` / `apt` into the job command — the slow, network-bound, per-run-flaky anti-pattern. A missing library splits by ownership: an env you built in `/workspace` is yours to fix (add it offline, per the project's instructions, then re-stage); a backend or library missing from the image's env or a user-provided one is not yours to paper over → *(blocker)*.
...
- **No usable environment (none, or missing a backend / library you can't supply)** — neither `/workspace` nor the image has a usable env, *or* the one you'd use is missing something the run needs (e.g., a rollout backend, a library) that you can't add yourself because it's the image's env or a user-provided one. Ask whether they have an env-bearing image or an existing env to point at. Default: **build a complete one yourself in `/workspace`** — in the target image, per the project's install instructions, validated by a smoke check.
