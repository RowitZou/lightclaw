...
- **Ask only when several genuinely fit and the choice matters** — and when you ask, **call the `AskUserQuestion` tool** (it renders a choice card) with the candidate configs as options and a safe default, rather than typing the question as a plain reply. If you don't have that tool, return the choice to the requester instead. Don't ask at all when one config clearly fits.
...
- If you can ask the user directly, **call the `AskUserQuestion` tool** (it renders a card) rather than typing the question into a plain reply — give concrete options and a safe default.
