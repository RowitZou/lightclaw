...
- Take avoidable failures off the table before you spend a submit: stage and validate the run first — assets, environment, network, capacity (see *Prepare and stage before you submit*).
...
3. **Assemble the spec from your library; default or ask for what's missing.**
   ...
   - **Environment** — in order: reuse a working env already in `/workspace`; else try the env the image ships and use it if it works; only if neither is usable is it a *(blocker)* — ask the user whether they have an existing env to point at, default **build it yourself in `/workspace`** (see *Prepare and stage before you submit*). Don't jump straight to building a whole env when a usable one is already there.
...
- **Settle the environment before you submit — and never let the job install it.** Take what already works, in order: an env in `/workspace`, then the env the image ships; build one yourself only when neither is usable (the *No environment* blocker's default) — for a GPU / training env, from a prep job in the target image, into `/workspace`. Use the image's env directly — don't link or copy it into `/workspace`: a `/workspace` env that's secretly a `--system-site-packages` venv over the image's conda only resolves inside that image (it lists but cannot run outside it), so a `/workspace` env you build must be self-contained. Whatever you land on, finish it *before* the run by preparing it offline — never bake `pip install` / `apt` into the job command, the slow, network-bound, per-run-flaky anti-pattern you're avoiding. If a run later hits a missing library: an env you built in `/workspace` is yours to fix — add it offline and re-stage; the image's env or a user-provided one is not — that's a *(blocker)*, ask the user.
...
- **No environment anywhere** — neither `/workspace` nor the image has a usable env. Ask whether they have an existing env (or an env-bearing image) to point at. Default: **build it yourself in `/workspace`.**
