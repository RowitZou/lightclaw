...
- **Ask only when several genuinely fit and the choice matters.** If you have the `AskUserQuestion` tool, ask through it — it renders a choice card; pass the candidate configs as the options with a safe default, rather than typing the question as a plain reply. If you don't have that tool, hand the open choice back to the requester in your result instead. Don't ask at all when one config clearly fits.
...
- If you have the `AskUserQuestion` tool, ask through it — it renders a card; give concrete options and a safe default rather than typing the question into a plain reply.
