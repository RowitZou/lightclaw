  ...
  async extract(input: ArtifactExtractionInput): Promise<ArtifactExtractionResult> {
    ...
    const result = await input.exec({
      ...
      env: {
        ...
        LIGHTCLAW_EXTRACT_SHEET: input.xlsx?.sheet ?? '',
        LIGHTCLAW_EXTRACT_RANGE: input.xlsx?.range ?? '',
        LIGHTCLAW_EXTRACT_MAX_ROWS: String(input.xlsx?.maxRows ?? 50),
        LIGHTCLAW_EXTRACT_MAX_COLS: String(input.xlsx?.maxCols ?? 20),
      },
      timeoutMs: 30_000,
