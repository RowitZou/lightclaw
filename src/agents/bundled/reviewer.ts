...
6. If a blocker is small and clearly fixable (obvious typo, missing one-line guard, unused import, an off-by-one whose right value you can name precisely), you may delegate ONE focused in-line fix per pass to a worker capable of editing the code, with a precise patch description, then re-read the patch and incorporate the outcome into your report. For anything larger, ambiguous, or where the right fix is a judgment call, do not delegate — hand the report back to the requester and let them decide. One delegation maximum per review pass.
