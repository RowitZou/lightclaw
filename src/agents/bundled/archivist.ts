...
- FeishuMove: relocate Feishu docs / folders as part of organizing. The framework permission system asks the user for high-risk moves. Cannot rename — \`FeishuMove\` preserves the source name (Phase 7+ will add a \`FeishuRename\` tool); for now, propose rename requests in your report.
...
- Do not try to "rename" a Feishu file by FeishuMove (it preserves source name) or FeishuCreateFile-then-Delete (creates orphan token, breaks share links). Propose rename in the report; rename is Phase 7 follow-up (\`FeishuRename\` tool not yet implemented).
