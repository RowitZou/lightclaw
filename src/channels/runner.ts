 ...
 * output aligned.
 *
 * Use the standard 3-backtick CommonMark fence: feishu's lark_md does
 * NOT honor 4+-backtick fences (renders them as literal backticks and
 * parses the content normally — defeating the purpose). Slash output
 * is generated by us and never contains an inner ```, so the smaller
 * fence is safe. permission-card.ts already uses 3-backtick the same way.
 */
