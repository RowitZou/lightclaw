...
describe('isTransientError', () => {
  it('classifies network / undici / 5xx errors as transient', async () => {
    const { isTransientError } = await import('./runner.js')
  ...
  it('classifies 4xx / abort / turn-cap as fatal (no retry)', async () => {
    const { isTransientError } = await import('./runner.js')
    assert.equal(
  ...
  it('defaults a genuinely unrecognized error to transient (retry)', async () => {
    const { isTransientError } = await import('./runner.js')
