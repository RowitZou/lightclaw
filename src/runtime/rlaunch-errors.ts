...
const RULES: ErrorRule[] = [
  ...
  },
  {
    ...
    suggestion: '调高 runtime.clusterSettings.memoryMb，或减少工具执行的内存占用。',
  },
  {
    ...
    suggestion: '确认 runtime.clusterSettings.image 已推到集群可访问 registry，且 imagePullPolicy / 凭证正确。',
  },
  {
    pattern: /namespaces? .*not found|namespace.*not found/i,
    user: '集群 namespace 配置不存在。',
    admin: 'RlaunchRuntime namespace not found',
    suggestion: '检查 runtime.clusterSettings.namespace 拼写和当前账号项目权限。',
  },
  {
    ...
    suggestion: '检查 runtime.clusterSettings.chargedGroup 拼写，确认配额组在目标 namespace 下存在。',
  },
  {
