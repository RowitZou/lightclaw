...
export function userCodexAuthDir(canonicalUser: string): string {
  return path.join(userHome(canonicalUser), 'auth', 'codex')
}
...
export function listUserCodexAuth(canonicalUser: string): UserCodexAuthSummary[] {
  ...
  const names = statSafeEntries(dir)
  ...
  if (!stored) {
    throw new AuthError({
      ...
      message:
        ...
        `Run /auth codex import --from <path> --name ${name}.`,
    })
  }
...
function statSafeEntries(dir: string): string[] {
  return Array.from(new Set(
    (existsSync(dir)
      ? readdirJsonBasenames(dir)
      : []),
  ))
}
...
function readdirJsonBasenames(dir: string): string[] {
  return readdirSync(dir, { withFileTypes: true })
    ...
    .map((entry: { name: string }) => entry.name.slice(0, -'.json'.length))
    .filter((name: string) => AUTH_NAME_RE.test(name))
}
